...
**Trampa de las seasons de API-Football (dos caras):** el flag `current` MIENTE en ligas muertas (Paraguay 250 siguió `current` meses después de cerrar — por eso se resuelve por fechas reales), pero la API también puede tener el `end` de la season VENCIDO con fixtures aún activos bajo esa misma season (medido con Venezuela, 8-sep-2026: end 7-sep, partidos programados después). El resolvedor compartido (`resolverSeason` en el cocinero) lleva fallback de frontera: si ninguna season contiene hoy, usa la de end más reciente dentro de 21 días, anunciándolo en el log. Ojo aparte: api/fixtures.js (el calendario público) aún resuelve con `current=true` — dos resolvedores, dos verdades posibles.
